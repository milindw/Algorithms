...
void DijkstraMinPath::ReadInputFile(const string& sInputFile)
{
    ...
    while(getline(ips, sLine))
    {
        ...
        ssWeightedEdges >> EdgeVertex;
        EdgeVertex--;

        ssWeightedEdges >> EdgeWeight;

        //cout << Vertex+1 << " " << EdgeVertex+1 << " " << EdgeWeight << "\n";
        m_AdjList[Vertex].m_AdjVertices.push_back(WeightedVertex{EdgeVertex, EdgeWeight});
        m_AdjList[Vertex].nVertex = Vertex;
        m_AdjList[EdgeVertex].m_AdjVertices.push_back(WeightedVertex{Vertex, EdgeWeight});
        m_AdjList[EdgeVertex].nVertex = EdgeVertex;
    }
...
void DijkstraMinPath::CalcMinPath()
{
    ...
    while(m_GreedyScore.size() != 0)
    {
        ...
        //cout << "Got " << MinVertex+1 << " " << MinWeight << "\n";
        ...
        for(auto it = m_AdjList[MinVertex].m_AdjVertices.begin(); it != m_AdjList[MinVertex].m_AdjVertices.end(); ++it)
        {
            ...
            for(auto itg = m_GreedyScore.begin(); itg != m_GreedyScore.end(); ++itg)
            {
                if((*itg).nVertex == (*it).nVertex)
                {
                    ...
                    if((*it).nWeight < (*itg).nWeight)
                    {
                        m_GreedyScore.erase(itg);
                        m_GreedyScore.insert( WeightedVertex {(*it).nVertex , (*it).nWeight});
                    }
                    break;
                }
            }
